<template>
	<view class="get">
		<view class="get-list">
			<uni-swipe-action>
				<uni-swipe-action-item class="row" v-for="(item, index) in getList" :right-options="options"
					:key="item.id" @click="swipeClick($event, item.id)">
			...
			<!-- <view v-for="(item, index) in getList" :key="index" class="row" @tap="navTo(`/pages/mycenter/log/detail?id=${item.id}`)">
				<view class="carrier">
					<view class="get-wrapper">
						<view class="title in1line">
		...
		<view class="add-round bg-deepBlue" :class="'bg-' + pageStore.$state.themeColor.name"
			@click="navTo(`/pages/mycenter/log/add`)">
...
<script>
	...
	export default {
		...
		methods: {
			// 数据初始化
			initData() {
				this.hasLogin = this.pageStore.$state.hasLogin;
				this.page = 1;
			...
			swipeClick(e, id) {
				...
				if (content.text === '删除') {
					...
					this.navTo(`/pages/mycenter/log/detail?id=${id}`);
				}
			},
			navTo(route) {
				this.$mRouter.push({
