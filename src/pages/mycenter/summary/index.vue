<template>
	<view class="get">
		...
		<view class="add-round bg-deepBlue" :class="'bg-' + pageStore.$state.themeColor.name" @click="navTo(`/pages/mycenter/summary/add`)">
...
<script>
	// import {summary,summaryType} from '@/api/basic';
		...
		methods: {
			...
			swipeClick(e, id) {
				...
				if (content.text === '删除') {
					...
					this.navTo(`/pages/mycenter/summary/detail?id=${id}`);
				}
			},
			navTo(route) {
				this.$mRouter.push({
