<template>
	<view class="get">
		...
		<view class="add-round bg-deepBlue" :class="'bg-' + pageStore.$state.themeColor.name" @click="navTo(`/pages/mycenter/calendar/add`)">
		...
		methods: {
			...
			swipeClick(e, id) {
				...
				if (content.text === '删除') {
					...
					this.navTo(`/pages/mycenter/calendar/detail?id=${id}`);
				}
			},
			navTo(route) {
				this.$mRouter.push({
